...
for line in analyzed_file:
  ...
  # analysis
  if re.search("testlog", line) and (not re.search("duplicates", line) or interval) and re.search("acv: " + acv, line) and re.search("time: 7200000", line):
    ...
    if m:
      ...
      if transmissions > 0:
        ...
        if route == "rpl":
          ...
          rpl_avg_tran_list.append(transmissions)
        elif route == "orpl":
          orpl_avg_dup_list.append(dup_percentage)
          orpl_avg_tran_list.append(transmissions)
        else:
...
print("rpl_duplicates: " + str(np.mean(rpl_avg_dup_list)) + ", rpl_duplicates_std: " + str(np.std(rpl_avg_dup_list)) + ", orpl_duplicates: " + str(np.mean(orpl_avg_dup_list)) + ", orpl_duplicates_std: " + str(np.std(orpl_avg_dup_list)) + ", rpl_transmissions: " + str(np.mean(rpl_avg_tran_list)) + ", rpl_transmissions_std: " + str(np.std(rpl_avg_tran_list)) + ", orpl_transmissions: " + str(np.mean(orpl_avg_tran_list)) + ", orpl_transmissions_std: " + str(np.std(orpl_avg_tran_list))) 
