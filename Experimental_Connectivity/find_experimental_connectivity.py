...
def print_usage():
  print("Correct usage:")
  print("  python find_experimental_connectivity.py acv_start acv_stop acv_step acv_log_path contiki_dir dag_num")
  exit()
...
for acv in np.arange(acv_start, acv_stop, acv_step):
  ...
  while search == True:
    ...
    if tup in connectivity_dict:
      ...
      output = subprocess.check_output(["python", "calc_experimental_connectivity.py", str(density), str(acv), contiki_dir, dag_num])
    ...
    if connectivity == 100.0:
      ...
      if density + 1 > 99:
        ...
        if tup_right in connectivity_dict:
          ...
          output = subprocess.check_output(["python", "calc_experimental_connectivity.py", str(density + 1), str(acv), contiki_dir, dag_num])
          m = re.search("Connected (\d*\.\d*)%, nodes: 100, density: (\d+), acv: (\d+\.\d+)%", output)
          if m:
            log.write(output)
